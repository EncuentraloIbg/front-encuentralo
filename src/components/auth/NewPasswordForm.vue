    ...
    <v-main class="auth-hero">
      <div class="auth-container">
        ...
        <form class="card" @submit.prevent="onSubmit">
          ...
          <button class="submit" type="submit" :disabled="loading || !isValid">
...
const isValid = computed(() => password.value.length >= 6 && password.value === confirm.value)
...
const onSubmit = async () => {
  ...
  if (!isValid.value) {
    message.value = 'La contraseña no cumple requisitos o no coincide.'
    messageType.value = 'error'
    return
  }
  ...
  loading.value = true
  try {
    const res: ApiMessage = await authService.resetPassword(token, password.value)
    message.value = res.message
    messageType.value = 'success'
  } catch (e) {
    const err = e as Error
    message.value = err?.message || 'No se pudo actualizar la contraseña.'
    messageType.value = 'error'
  } finally {
    loading.value = false
  }
...
<style scoped>
/* Reutiliza exactamente los estilos de ForgotPassword.vue (puedes extraerlos a un .css si prefieres) */
...
.logo{ height:clamp(82px,11vw,135px); width:auto; display:inline-block; object-fit:contain; }
...
.card{ width:100%; margin-top:8px; padding:24px 22px 20px; border-radius:16px; background:var(--glass); -webkit-backdrop-filter:blur(12px); backdrop-filter:blur(12px); border:1px solid var(--glass-border); box-shadow:0 12px 30px rgba(0,0,0,.28); }
.alert{ padding:10px 12px; border-radius:10px; margin-bottom:12px; font-size:14px; text-align:center; border:1px solid rgba(255,255,255,.35); background:rgba(255,255,255,.25); color:#fff; }
.alert.success{ border-color:rgba(76,175,80,.55); box-shadow:0 6px 16px rgba(76,175,80,.22); }
.alert.error{ border-color:rgba(198,40,40,.55); box-shadow:0 6px 16px rgba(198,40,40,.22); }
.input-group{ margin-bottom:16px; text-align:center; }
label{ display:block; font-size:13px; color:#eaeef6; margin-bottom:6px; }
input{ width:100%; padding:12px 14px; border:1px solid rgba(255,255,255,.55); border-radius:10px; outline:none; background:rgba(255,255,255,.96); color:#111; font-size:15px; text-align:center; transition:box-shadow .2s ease, border-color .2s ease, background .2s ease; }
input::placeholder{ color:#6b7280; }
input:focus{ border-color:#fff; box-shadow:0 0 0 3px rgba(21,101,192,.25); background:#fff; }
.submit{ width:100%; padding:12px 14px; border:none; border-radius:10px; background:#111; color:#fff; font-weight:700; font-size:15px; cursor:pointer; transition:transform .15s ease, box-shadow .25s ease, background .2s ease; }
